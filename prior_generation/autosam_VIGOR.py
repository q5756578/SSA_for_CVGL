...
def process_prompt_by_subfolder(prompt, subfolder_images, target_base_path, num_gpus=4):
    ...
    for subfolder_name, image_files in subfolder_images.items():
        if subfolder_name:
            ...
            subfolder_target_path = target_path
        ...
        print(f"Completed processing {'subfolder '+subfolder_name if subfolder_name else 'main folder'}")
...
def main():
    ...
    text_prompt_list = ['construction', 'edifice','artificial buildings','structures', 'trees', 'construction']
    num_gpus = 4
    base_path = '/home/jshen/data/VIGOR'  # Base path for VIGOR dataset
    
    # Process each city
    for city in cities:
        print(f"\n=== Processing city: {city} ===")
        
        # Build satellite image path for current city
        image_path = os.path.join(base_path, city, 'satellite')
        
        # Get all images in the current city's satellite image folder
        subfolder_images = find_image_files_by_subfolder(image_path)
        if not subfolder_images:
            # If no subfolders, treat all images as one group
            image_files = []
            for ext in ('*.jpg', '*.jpeg', '*.png'):
                image_files.extend(glob.glob(os.path.join(image_path, ext)))
            if image_files:
                subfolder_images = {'': sorted(image_files)}
        
        total_images = sum(len(files) for files in subfolder_images.values())
        print(f"Total images to process in {city}: {total_images}")
        print(f"Subfolders found in {city}:", list(subfolder_images.keys()))
        
        # Set output base directory for current city
        output_base_path = os.path.join(base_path, city)
        ...
        print(f"\nCity {city} processing completed:")
