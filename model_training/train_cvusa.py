...
from sample4geo.dataset.cvusa_v2 import CVUSADatasetEval, CVUSADatasetTrain
from sample4geo.transforms import get_transforms_train, get_transforms_val, get_transforms_bev 
...
class Configuration:
    ...
    # Dataset
    data_folder = "/CVUSA/CVUSA_subset"     
...
if __name__ == '__main__':
    # 使用更详细的时间格式，包含月日时分秒
    current_time = time.strftime("%m%d_%H%M%S")  # 格式: MMDD_HHMMSS
    ...
    sys.stdout = Logger(os.path.join(model_path, log_filename))
    ...
    for epoch in range(1, config.epochs+1):
        ...
        if (epoch % config.eval_every_n_epoch == 0 and epoch != 0) or epoch == config.epochs:
            ...
            r_results = evaluate(config=config,
                               model=model,
                               reference_dataloader=reference_dataloader_test,
                               query_dataloader=query_dataloader_test, 
                               ranks=[1, 5, 10],
